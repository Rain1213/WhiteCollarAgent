"""Diagnostic environment for the "switch to GUI mode" action."""
...
def _build_internal_interface_stub() -> tuple[types.ModuleType, type]:
    ...
    class InternalActionInterface:  # noqa: D401
        ...
        @staticmethod
        def switch_to_GUI_mode() -> None:
            InternalActionInterface.gui_mode = True

        @staticmethod
        def switch_to_CLI_mode() -> None:  # pragma: no cover - not used here
...
def _prepare_switch_to_gui(
    ...
    return PreparedEnv(
        ...
        context={"interface_cls": interface_cls},
    )


def _validate_switch_to_gui(
    ...
    if payload.get("gui_mode") != EXPECTED_MODE:
        return "incorrect result", "Reported gui_mode did not match expected GUI mode."

    interface_cls = context.get("interface_cls")
    if getattr(interface_cls, "gui_mode", None) != EXPECTED_MODE:
        return "incorrect result", "Internal interface did not switch to GUI mode."

    return "passed", "Agent switched to GUI mode successfully."
...
def get_test_case() -> ActionTestCase:
    return ActionTestCase(
        ...
        base_input={},
        prepare=_prepare_switch_to_gui,
        validator=_validate_switch_to_gui,
    )
